...
function Resolve-DynamicTokenArray {
    ...
    if ($Value -is [System.Collections.IEnumerable] -and -not ($Value -is [string])) {
        ...
        foreach ($elem in $Value) {
            $sub = Resolve-DynamicTokenArray -Value $elem -Report $Report -DefaultDateFormat $DefaultDateFormat -BasePath $BasePath
    ...
    # 1. Check for external text file parameter (@path or {file:path})
    $isFromFile = $false
    $filePathRaw = ''

    if ($valStr -match '^\{file:\s*(.+?)\s*\}$') {
        $isFromFile = $true
        $filePathRaw = $matches[1]
    }
    elseif ($valStr.StartsWith('@')) {
        $isFromFile = $true
        $filePathRaw = $valStr.Substring(1).Trim('"', "'", ' ')
    }

    if ($isFromFile) {
        $resolvedFilePath = Resolve-DynamicTokens -Text $filePathRaw -Report $Report -DefaultDateFormat $DefaultDateFormat
        if (-not [System.IO.Path]::IsPathRooted($resolvedFilePath) -and -not [string]::IsNullOrWhiteSpace($BasePath)) {
        ...
        if (-not (Test-Path -LiteralPath $resolvedFilePath)) {
            throw "Không tìm thấy tệp tham số: '$resolvedFilePath' (từ cấu hình '$filePathRaw')"
        }
        ...
        foreach ($line in $lines) {
            ...
            # Check for comma / semicolon separated items (and not ISO date like yyyy-MM-dd)
            if ($trimmedLine -match '[,;]' -and $trimmedLine -notmatch '^\d{4}-\d{2}-\d{2}') {
                ...
                foreach ($st in $subTokens) {
                    ...
                    if (-not [string]::IsNullOrWhiteSpace($stTrimmed)) {
                        $subResolved = Resolve-DynamicTokenArray -Value $stTrimmed -DefaultDateFormat $DefaultDateFormat -BasePath $BasePath
                        foreach ($sr in $subResolved) { [void]$fileItems.Add($sr) }
                    }
                }
            }
            else {
                $lineResolved = Resolve-DynamicTokenArray -Value $trimmedLine -DefaultDateFormat $DefaultDateFormat -BasePath $BasePath
                foreach ($lr in $lineResolved) { [void]$fileItems.Add($lr) }
            }
        }
    ...
    # 2. Check for Multi-EOM / Multi-EOQ Range Tokens:
    ...
    if ($valStr -match $rangePattern) {
        $tokenType = $matches[1].ToUpperInvariant()
        ...
        if ($tokenType -in @('ALLEOM_YTD', 'EOM_YTD')) {
            for ($m = 1; $m -lt $curMonth; $m++) {
                $daysInM = [DateTime]::DaysInMonth($curYear, $m)
                [void]$dates.Add((New-Object DateTime($curYear, $m, $daysInM)))
            }
        }
        elseif ($tokenType -in @('ALLEOM_CURRENTYEAR', 'ALLEOM_YEAR', 'EOM_YEAR')) {
            for ($m = 1; $m -le 12; $m++) {
                $daysInM = [DateTime]::DaysInMonth($curYear, $m)
                [void]$dates.Add((New-Object DateTime($curYear, $m, $daysInM)))
            }
        }
        elseif ($tokenType -in @('ALLEOM_LASTYEAR', 'ALLEOM_PREVIOUSYEAR')) {
            $prevYear = $curYear - 1
            for ($m = 1; $m -le 12; $m++) {
                $daysInM = [DateTime]::DaysInMonth($prevYear, $m)
                [void]$dates.Add((New-Object DateTime($prevYear, $m, $daysInM)))
            }
        }
        elseif ($tokenType -in @('ALLEOM_LAST12M', 'EOM_LAST12M')) {
            for ($i = 12; $i -ge 1; $i--) {
                $refDate = $today.AddMonths(-$i)
                $daysInM = [DateTime]::DaysInMonth($refDate.Year, $refDate.Month)
                [void]$dates.Add((New-Object DateTime($refDate.Year, $refDate.Month, $daysInM)))
            }
        }
        elseif ($tokenType -in @('ALLEOQ_YTD', 'EOQ_YTD')) {
            $curQuarter = [int][Math]::Ceiling($curMonth / 3.0)
            for ($q = 1; $q -lt $curQuarter; $q++) {
                $endMonth = $q * 3
                $daysInM = [DateTime]::DaysInMonth($curYear, $endMonth)
                [void]$dates.Add((New-Object DateTime($curYear, $endMonth, $daysInM)))
            }
        }
        elseif ($tokenType -in @('ALLEOQ_CURRENTYEAR', 'ALLEOQ_YEAR')) {
            for ($q = 1; $q -le 4; $q++) {
                $endMonth = $q * 3
                $daysInM = [DateTime]::DaysInMonth($curYear, $endMonth)
                [void]$dates.Add((New-Object DateTime($curYear, $endMonth, $daysInM)))
        ...
        elseif ($tokenType -in @('ALLEOQ_LASTYEAR', 'ALLEOQ_PREVIOUSYEAR')) {
            $prevYear = $curYear - 1
            for ($q = 1; $q -le 4; $q++) {
                $endMonth = $q * 3
                $daysInM = [DateTime]::DaysInMonth($prevYear, $endMonth)
                [void]$dates.Add((New-Object DateTime($prevYear, $endMonth, $daysInM)))
            }
        }
        elseif ($tokenType -in @('ALL_EOH_CURRENTYEAR', 'ALLEOH_YEAR')) {
            [void]$dates.Add((New-Object DateTime($curYear, 6, 30)))
            [void]$dates.Add((New-Object DateTime($curYear, 12, 31)))
        }
        elseif ($tokenType -eq 'ALLEOH_LASTYEAR') {
            $prevYear = $curYear - 1
            [void]$dates.Add((New-Object DateTime($prevYear, 6, 30)))
            [void]$dates.Add((New-Object DateTime($prevYear, 12, 31)))
        }
    ...
    # 3. Check for comma / semicolon separated values in inline strings
    $strTrimmed = $valStr.Trim()
    if ($strTrimmed -match '[,;]' -and $strTrimmed -notmatch '^\d{4}-\d{2}-\d{2}') {
        ...
        foreach ($sub in $subItems) {
            $evalSub = Resolve-DynamicTokens -Text $sub -DefaultDateFormat $DefaultDateFormat
    ...
    $single = Resolve-DynamicTokens -Text $valStr -DefaultDateFormat $DefaultDateFormat
    return ,@($single)
}
...
function Get-ObieePromptedFilters {
    <#
    .SYNOPSIS
        Lấy danh sách prompted filters từ báo cáo qua xmlViewService.getPromptedFilters.
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)] [string]$BaseUrl,
    ...
    $endpoint = $BaseUrl.TrimEnd('/')
    if ($endpoint -notmatch '\?SoapImpl=') {
        $endpoint += "?SoapImpl=xmlViewService"
    }

    $body = @"
        <sawsoap:getPromptedFilters>
            <sawsoap:report>
...
"@
    ...
    $xml = Invoke-ObieeSoapRequest -ServiceUrl $endpoint -SoapBodyContent $body
    $colNodes = $xml.SelectNodes("//*[local-name()='column' or local-name()='promptedFilter']")
    $results = New-Object System.Collections.Generic.List[string]

    if ($null -ne $colNodes) {
        foreach ($node in $colNodes) {
            $colName = $node.InnerText
            if (-not [string]::IsNullOrWhiteSpace($colName)) {
                $results.Add($colName.Trim())
            }
        }
    }
...
function Get-ObieeReportParameters {
    <#
    .SYNOPSIS
        Tự động kiểm tra và trích xuất danh sách biến trình bày (@{varName}{default})
        và bộ lọc được yêu cầu (prompted filters) từ báo cáo OBIEE.
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)] [string]$BaseUrl,
    ...
    $paramList = New-Object System.Collections.Generic.List[PSCustomObject]
    $seen = New-Object System.Collections.Generic.HashSet[string]

    # 1. Đọc XML định nghĩa báo cáo qua readObject (Catalog Service)
    try {
        $catalogXml = Read-ObieeObject -BaseUrl $BaseUrl -Path $Path -SessionId $SessionId
        if (-not [string]::IsNullOrWhiteSpace($catalogXml)) {
            # Quét các biến dạng @{varName}{default} hoặc @{varName}
            $varMatches = [regex]::Matches($catalogXml, '@\{([a-zA-Z0-9_\.\-]+)\}(?:\{([^}]*)\})?')
            foreach ($m in $varMatches) {
                $vName = $m.Groups[1].Value
                $vDef = if ($m.Groups[2].Success) { $m.Groups[2].Value } else { '{Yesterday}' }
                if (-not $seen.Contains($vName)) {
                    [void]$seen.Add($vName)
                    [void]$paramList.Add([pscustomobject]@{
                        Name         = $vName
                        DefaultValue = $vDef
                        Source       = 'PresentationVariable'
                        Description  = "Biến trình bày trong phân tích (@{$vName})"
                    })
                }
            }

            # Quét các prompted filters dạng <saw:filter ...> có prompt
            $filterMatches = [regex]::Matches($catalogXml, '(?i)<saw:filter[^>]*?formula="([^"]+)"[^>]*?>')
            foreach ($m in $filterMatches) {
                $formula = $m.Groups[1].Value
                if ($formula -match '@\{([a-zA-Z0-9_\.\-]+)\}') {
                    $vName = $matches[1]
                    if (-not $seen.Contains($vName)) {
                        [void]$seen.Add($vName)
                        [void]$paramList.Add([pscustomobject]@{
                            ...
                            Source       = 'FilterVariable'
                            Description  = "Biến trong điều kiện lọc (@{$vName})"
                        })
                    }
                }
            }
        ...
        Write-Log "Lỗi khi đọc Catalog Object XML: $($_.Exception.Message)" 'DEBUG'
    }

    # 2. Gọi getPromptedFilters từ xmlViewService
    try {
        $promptCols = Get-ObieePromptedFilters -BaseUrl $BaseUrl -Path $Path -SessionId $SessionId
        if ($null -ne $promptCols) {
            foreach ($c in $promptCols) {
                if ($c -match '@\{([a-zA-Z0-9_\.\-]+)\}') {
                    ...
                    if (-not $seen.Contains($vName)) {
                        ...
                        [void]$paramList.Add([pscustomobject]@{
                            Name         = $vName
                            DefaultValue = '{Yesterday}'
                            Source       = 'PromptedFilter'
                            Description  = "Biến trong bộ lọc prompted (@{$vName})"
                        })
                    }
                }
            }
        ...
        Write-Log "Lỗi khi gọi getPromptedFilters: $($_.Exception.Message)" 'DEBUG'
    }
